...
mod ERC404Component {
    ...
    struct Storage {
        ...
        ERC404_minted: u256,
        // Token balance of user in fractional representation
    ...
    mod Errors {
        ...
        const NOT_ENOUGH_BALANCE: felt252 = 'ERC404: not enough balance';
    ...
    > of interface::IERC404<ComponentState<TContractState>> {
        ...
        fn transfer_from(
            ...
            if (amount_or_id <= self.ERC404_minted.read() && amount_or_id > 0) {
                ...
                // Check balances
                assert(self.ERC404_balances.read(from) >= uint, Errors::NOT_ENOUGH_BALANCE);

                // Check token allowance

                self.ERC404_balances.write(from, self.ERC404_balances.read(from) - uint);

                self.ERC404_balances.write(to, self.ERC404_balances.read(to) + uint);
                ...
                curRecipientIds.append(amount_or_id);
                ...
                self.emit(NFTTransfer { from: from, to: to, token_id: amount_or_id });
                self.emit(Transfer { from: from, to: to, value: uint });
        ...
        fn approve(
            ...
            if (amount_or_id <= self.ERC404_minted.read() && amount_or_id > 0) {
                ...
                self.emit(Approval { owner: caller, spender: spender, value: amount_or_id });
            }
    ...
    > of InternalTrait<TContractState> {
        ...
        fn initializer(
            ...
            self.ERC404_balances.write(owner, self.ERC404_total_supply.read());
        ...
        fn _transfer(
            ...
            let balanceBeforeSender = self.ERC404_balances.read(from);
            let balanceBeforeReceiver = self.ERC404_balances.read(to);

            assert(balanceBeforeSender >= amount, Errors::NOT_ENOUGH_BALANCE);
            ...
            if (!self.ERC404_whitelist.read(from)) {
                let tokens_to_burn = (balanceBeforeSender / uint)
                    - (self.ERC404_balances.read(from) / uint);
            ...
            if (!self.ERC404_whitelist.read(to)) {
                let tokens_to_mint = (self.ERC404_balances.read(to) / uint)
        ...
        fn _burn(ref self: ComponentState<TContractState>, from: ContractAddress) {
            ...
            self.emit(NFTTransfer { from: from, to: Zeroable::zero(), token_id: id });
        }
    }
